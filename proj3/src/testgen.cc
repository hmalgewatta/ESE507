...
int main(int argc, char* argv[]) {
    ...
    // Mode 3: Generate a testbench for a network of three convolutions. 
        ...
        /*
        // For debugging:
        // Store intermediateVals1 (the outputs from layer 1) to a file called tb_net_N_M1_M2_M3_T_A_layer1.exp in decimal
        expResFileName = "tb_net_" + to_string(N) + "_" + to_string(M1) + "_" + to_string(M2) + "_" + to_string(M3) + "_" + to_string(T) + "_" + to_string(mult_budget) + "_layer1.exp";
        os.open(expResFileName);
        if (!os.is_open()) {
            cout << "ERROR opening " << expResFileName << " for writing." << endl;
            return 1;
        }
        os << "For debugging: expected decimal outputs corresponding to Layer 1" << endl;
        for (int i=0; i<intermediateVals1.size(); i++) {
            os << intermediateVals1[i] << endl;
        }
        os.close();

        // For debugging:
        // Store intermediateVals2 (the outputs from layer 2) to a file called tb_net_N_M1_M2_M3_T_A_layer2.exp in decimal
        expResFileName = "tb_net_" + to_string(N) + "_" + to_string(M1) + "_" + to_string(M2) + "_" + to_string(M3) + "_" + to_string(T) + "_" + to_string(mult_budget) + "_layer2.exp";
        os.open(expResFileName);
        if (!os.is_open()) {
            cout << "ERROR opening " << expResFileName << " for writing." << endl;
            return 1;
        }
        os << "For debugging: expected decimal outputs corresponding to Layer 2" << endl;
        for (int i=0; i<intermediateVals2.size(); i++) {
            os << intermediateVals2[i] << endl;
        }
        os.close();
        */
