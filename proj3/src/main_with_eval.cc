...
void genAllLayersWithUserSpec(int N, int M1, int M2, int M3, int T, int A, int L1, int L2, int L3, vector<int> &constVector, string modName, ofstream &os);
...
int main(int argc, char *argv[])
{
    ...
    else if ((mode == 4) && (argc == 12))
    {
        ...
        // call the genAllLayers function
        genAllLayersWithUserSpec(N, M1, M2, M3, T, A, P1, P2, P3, constVector, mod_name, os);
    }
        ...
        for (j = 1; j <= L2; ++j)
        {
            for (k = 1; k <= std::min(L3, std::max(B - i - j, 0)); k++)
            {
                if (k <= 0 || L2 % j != 0 || L3 % k != 0)
                    continue;
                temp = N + tMac * L1 / i + L1 + tMac * L2 / j + L2 + tMac * L3 / k + L3;
...
void includeParameterizedTopModule(int T, string modName, string subModName1, string subModName2, string subModName3, ofstream &os)
{
    string templateFile = "3layer_template.sv";
...
void genAllLayersWithUserSpec(int N, int M1, int M2, int M3, int T, int A, int P1, int P2, int P3, vector<int> &constVector, string modName, ofstream &os)
{
